...
Every phase record has typed input, result, status, timestamps, and failure state. Completed expensive work is not repeated on resume. **No edits occur before `CONTRACT_LOCK`.**
...
Builder and evaluator must independently accept the **same canonical JSON hash** before edits. Amendments must be a complete replacement contract. Conflicting amendments are persisted and do not lock. Dual-review failure stops safely with no edits.
...
- `request.json`, `preflight.json`, `model.json`, `role-permissions.json`
- `artifacts/prompts/` exact composed prompts
- `page-map.json`, `baseline.json`, `config.json`
- `inspect/before|after`, `performance/*`
- contract proposals, decisions, conflicts, canonical contract, hash
...
Defaults: 3 repair iterations, 90 minutes, 3 contract rounds, 40 agent runs, 2_000_000 tokens. Crossing a limit cancels safely, preserves the best **passing** checkpoint (or baseline), and records why the run stopped.

Use `--max-iterations 1` and `--risk low` for cheap probes. `page:inspect` and `page:routes` spend no model tokens.
