...
| **Env** | Prefer `AI_GATEWAY_API_KEY` or Vercel OIDC (`VERCEL_OIDC_TOKEN`). Direct `ANTHROPIC_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` still work as fallbacks |
...
| **Cron** | `/api/cron/tick` and `/api/cron/worker` every 5 minutes (`vercel.json`) enqueue editions and refresh tape/news. `/api/cron/intel` every 15 minutes refreshes session/book briefs. Brokerage sync uses Supabase `pg_cron`, not Vercel. |
