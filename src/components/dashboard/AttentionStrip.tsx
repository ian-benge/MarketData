    ...
    >
      <ol className="flex divide-x divide-[var(--ib-border-subtle)] overflow-x-auto terminal-scroll">
        {items.map((item) => {
          ...
          return (
            <li key={item.id} className="min-w-[9.5rem] flex-1 sm:min-w-[11.5rem]">
              {clickable ? (
                <button
                  type="button"
                  onClick={() => onSelectSymbol?.(item.ticker!)}
                  className="h-full w-full px-2.5 py-1.5 text-left hover:bg-[var(--ib-surface-hover)] sm:px-3 sm:py-2"
                  aria-label={`Open ${item.ticker} in primary chart`}
                >
                  {inner}
                </button>
              ) : (
                <div className="px-2.5 py-1.5 sm:px-3 sm:py-2">{inner}</div>
              )}
            </li>
          );
        })}
