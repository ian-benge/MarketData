"use client";

import Link from "next/link";
import { Badge } from "@/components/ui/Badge";
import { Panel } from "@/components/ui/Panel";
import type { DashboardBookImpact } from "@/lib/dashboard/book-impact";
import { cn } from "@/lib/utils/cn";
import {
  formatSignedCurrency,
  formatSignedPercent,
  marketToneClass,
} from "@/lib/utils/format";

function weightLabel(weight: number | null) {
  if (weight == null || !Number.isFinite(weight)) return null;
  const pct = weight <= 1 ? weight * 100 : weight;
  return `${Math.round(pct)}% max wt`;
}

export function BookImpactStrip({
  book,
  onSelectSymbol,
}: {
  book: DashboardBookImpact;
  onSelectSymbol?: (ticker: string) => void;
}) {
  const empty = book.openCount === 0;
  const unavailable = book.persistence === "unavailable";
  const extraUnexplained = book.unexplainedTickers.filter(
    (ticker) =>
      !book.contributors.some((row) => row.ticker.toUpperCase() === ticker.toUpperCase()),
  );
  const description = book.ownerLocked
    ? "Names listed. P&L stays locked until the book is unlocked on Positions."
    : unavailable
      ? "Blotter is not connected. No P&L was invented."
      : empty
        ? undefined
        : "Session P&L from cached marks. Unexplained uses Material News attribution.";

  return (
    <Panel
      id="book-impact"
      title="Book impact"
      description={description}
      className="scroll-mt-[11.5rem]"
      bodyClassName="p-2.5"
      actions={
        <Link
          href="/positions"
          className="text-[11px] font-medium text-[var(--ib-maroon-300)] hover:underline"
        >
          Open Positions
        </Link>
      }
    >
      {book.error ? (
        <p className="mb-2 text-[12px] text-[var(--state-warning)]">{book.error}</p>
      ) : null}
      {empty && !book.error ? (
        <p className="text-[12px] text-[var(--ib-text-muted)]">
          {unavailable
            ? "Book unavailable — open Positions after persistence is connected."
            : "No open lots in the active book."}
        </p>
      ) : empty ? null : (
        <div className="flex min-w-0 flex-wrap items-stretch gap-2">
          <dl className="grid min-w-[13.5rem] flex-1 grid-cols-4 gap-1.5 font-mono text-[11px]">
            <div className="rounded-[4px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-inset)] px-2 py-1">
              <dt className="text-[9px] uppercase tracking-[0.08em] text-[var(--ib-text-muted)]">
                Day P&L
              </dt>
              <dd className={cn("mt-0.5 text-[13px] tabular-nums", marketToneClass(book.dayPnl))}>
                {book.ownerLocked ? "Locked" : formatSignedCurrency(book.dayPnl, { compact: true })}
              </dd>
            </div>
            <div className="rounded-[4px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-inset)] px-2 py-1">
              <dt className="text-[9px] uppercase tracking-[0.08em] text-[var(--ib-text-muted)]">
                Day %
              </dt>
              <dd className={cn("mt-0.5 text-[13px] tabular-nums", marketToneClass(book.dayPercent))}>
                {book.ownerLocked ? "Locked" : formatSignedPercent(book.dayPercent)}
              </dd>
            </div>
            <div className="rounded-[4px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-inset)] px-2 py-1">
              <dt className="text-[9px] uppercase tracking-[0.08em] text-[var(--ib-text-muted)]">
                Open
              </dt>
              <dd className="mt-0.5 text-[13px] tabular-nums text-[var(--ib-text-primary)]">
                {book.openCount}
                {book.unexplainedTickers.length ? (
                  <span className="ml-1 text-[10px] text-[var(--state-warning)]">
                    · {book.unexplainedTickers.length} unexplained
                  </span>
                ) : null}
              </dd>
            </div>
            <div className="rounded-[4px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-inset)] px-2 py-1">
              <dt className="text-[9px] uppercase tracking-[0.08em] text-[var(--ib-text-muted)]">
                Concentration
              </dt>
              <dd className="mt-0.5 text-[13px] tabular-nums text-[var(--ib-text-primary)]">
                {book.ownerLocked ? "Locked" : (weightLabel(book.largestWeight) ?? "—")}
              </dd>
            </div>
          </dl>
          {book.contributors.length || extraUnexplained.length ? (
            <ul className="flex min-w-0 flex-[2] flex-wrap content-center gap-1.5">
              {book.contributors.map((row) => (
                <li key={row.ticker}>
                  <button
                    type="button"
                    onClick={() => onSelectSymbol?.(row.ticker)}
                    className="inline-flex items-center gap-1.5 rounded-[3px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-2)] px-2 py-1 hover:border-[var(--ib-border-control)]"
                    aria-label={`Select ${row.ticker}`}
                  >
                    <span className="font-mono text-[11px] font-semibold text-[var(--ib-text-primary)]">
                      {row.ticker}
                    </span>
                    <span className="font-mono text-[10px] uppercase text-[var(--ib-text-muted)]">
                      {row.side}
                    </span>
                    <span className={cn("font-mono text-[11px] tabular-nums", marketToneClass(row.dayPercent))}>
                      {book.ownerLocked ? "—" : formatSignedPercent(row.dayPercent)}
                    </span>
                    {row.unexplained ? <Badge tone="warn">Unexplained</Badge> : null}
                  </button>
                </li>
              ))}
              {extraUnexplained.map((ticker) => (
                <li key={`unexplained-${ticker}`}>
                  <button
                    type="button"
                    onClick={() => onSelectSymbol?.(ticker)}
                    className="inline-flex items-center gap-1.5 rounded-[3px] border border-[color-mix(in_oklab,var(--state-warning)_38%,var(--ib-border-subtle))] bg-[var(--ib-surface-2)] px-2 py-1"
                    aria-label={`Select ${ticker}`}
                  >
                    <span className="font-mono text-[11px] font-semibold text-[var(--ib-text-primary)]">
                      {ticker}
                    </span>
                    <Badge tone="warn">Unexplained</Badge>
                  </button>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-[12px] text-[var(--ib-text-muted)]">
              No open lots in the active book.
            </p>
          )}
        </div>
      )}
      {book.usingFixtures ? (
        <p className="mt-1.5 text-[10px] text-[var(--ib-text-muted)]">DEMO fixture blotter.</p>
      ) : null}
    </Panel>
  );
}
