  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col rounded-[6px] border border-[var(--ib-border-subtle)] bg-[var(--ib-surface-inset)] px-3 pb-3 pt-3 sm:px-4 sm:pb-4 sm:pt-3">
      ...
      <div className="relative pt-7" aria-label={score == null ? "Market Pulse score unavailable" : `Market Pulse score ${score} out of 100, ${result.regime}`}>
        <div className="grid h-2 grid-cols-5 overflow-hidden rounded-full border border-[var(--ib-border-strong)]">
          {LABELS.map((label, index) => (
            <span
              key={label}
              className={cn(
                "border-r border-[var(--ib-border-strong)] last:border-r-0",
                index === 2
                  ? "bg-[var(--ib-surface-3)]"
                  : index < 2
                    ? "bg-[color-mix(in_oklab,var(--market-negative)_22%,var(--ib-surface-2))]"
                    : "bg-[color-mix(in_oklab,var(--market-positive)_22%,var(--ib-surface-2))]",
              )}
            />
          ))}
        </div>
        {score != null ? (
          <div className="absolute top-0 -translate-x-1/2" style={{ left: `${score}%` }}>
            <span className="block rounded-[3px] border border-[var(--ib-maroon-500)] bg-[var(--ib-surface-3)] px-1.5 py-0.5 font-mono text-[11px] font-semibold text-[var(--ib-text-primary)] shadow-[0_0_14px_color-mix(in_oklab,var(--ib-maroon-500)_28%,transparent)]">
              {score}
            </span>
        ...
        <div className="mt-1.5 grid grid-cols-5 gap-1">
          {LABELS.map((label) => (
            <span key={label} className="text-center font-mono text-[8px] leading-3 text-[var(--ib-text-muted)] sm:text-[9px]">
              {label}
            </span>
          ))}
        </div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[var(--ib-border-subtle)] pt-3 font-mono text-[9px] uppercase tracking-[0.08em] text-[var(--ib-text-muted)]">
        <span>0</span>
        <span>{score == null ? "Score withheld" : `Pulse score ${score} / 100`}</span>
        <span>100</span>
      </div>
    </div>
  );
