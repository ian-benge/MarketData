...
describe("redactLockedOwnerSnapshot", () => {
  it("keeps open tape and lot P&L, and strips account value, closed lots, and notes", () => {
    ...
    expect(open.quantity).toBe(0);
    expect(open.entryPrice).toBe(0);
    expect(open.last).toBeNull();
    ...
    expect(redacted.summary.unrealizedPnl).toBeNull();
    expect(redacted.summary.totalPnl).toBeNull();
    expect(redacted.summary.dayPnl).toBeNull();
    expect(redacted.summary.bookReturnPercent).toBeNull();
    expect(redacted.summary.dayPercent).toBeNull();
